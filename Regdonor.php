...
function passwordsMatch()
{
    ...
    if ($password == $confirmPassword && strlen($password) > 7) {
        checkEmailAndSendOtp($GLOBALS['conn']);
    } else {
        echo '<script>alert("Passwords do not match or are less than 8 characters.");</script>';
    }
}
...
function checkEmailAndSendOtp($conn)
{
    ...
    if (!preg_match('/^(19|20)\d\d-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$/', $dob)) {
        echo '<script>alert("Invalid Date Of Birth.");</script>';
        return;
    }

    // Check if the email is already registered
    $query = $conn->prepare("SELECT donorEmail FROM donormaster WHERE donorEmail = ?");
    $query->bind_param('s', $recipient_email);
    $query->execute();
    $query->store_result();

    $user = $conn->prepare("SELECT donorUserName FROM DonorMaster WHERE donorUserName = ?");
...
function sendotp()
{
    try {
        ...
        $mail->Password = 'mrbghyyulnvcuaqm'; // Enter email password
        $mail->SMTPSecure = 'tls';
        $mail->Port = 587;
...
function registerUser($conn)
{
    ...
    $stmt = $conn->prepare("INSERT INTO DonorMaster (donorUserName, donorContact, donorEmail, donorDob, donorStreet, donorArea, donorCity, donorPincode, donorPassword) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                            ...
                            <div id="donorForm">
                                <div class="row" style="height:1550px">
                                    <div class="col-lg-12 col-md-12 col-12">
                                        ...
                                        <input type="text" id="username" name="username" pattern="[A-Za-z\s]{1,50}"
                                            placeholder="Your Name here.."
                                            value="<?php echo htmlspecialchars($_SESSION['form_data']['username'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                                            required>
                                        <label for="contact">Contact</label>
                                        <input type="text" id="contact" name="contact" placeholder="Your Contact here.."
                                            value="<?php echo $_SESSION['form_data']['contact'] ?? ''; ?>"
                                            maxlength="10x"><br />
                                        <label for="email">Email</label>
                                        <input type="email" id="email" name="email" placeholder="Your email here.."
                                            value="<?php echo $_SESSION['form_data']['email'] ?? ''; ?>"><br />
                                    </div>
                                    ...
                                    <div class="button" colspan="2">
                                        <input type="submit" name="generate_otp" value="Generate OTP"
                                            onclick="validateForm()" class="tp-accountBtn">
                                    </div>
